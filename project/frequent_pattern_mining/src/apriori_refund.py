...
def apriori_process_sub(data_file, support_threshold, confidence_threshold):
	parquet_file = os.path.join(data_file, "data_refund.parquet")
	# n = 135000000
	n = 20000000
	...
	# 将频繁项集与关联规则储存至txt文件中
	rules_txt_file = os.path.join(data_file, "rules_refund.txt")
	print("\nAssociation rules:")
...
def apriori_process():
	print("Real data:")
	apriori_process_sub("data/30G_data_new", 0.005, 0.4)
