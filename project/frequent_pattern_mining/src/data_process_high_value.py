...
def data_process_high_value_sub(data_file, part_num, category_mp, price_mp):
	...
	for ind, parquet_file in enumerate(get_parquet_files(data_file, part_num)):
		...
		for item, payment_method in data[['items', 'payment_method']].values:
			item_list = []
			for item_id in item.tolist():
				if price_mp[item_id] > price_threshold:
					item_list.append(category_mp[item_id])
			if len(item_list) > 0:
				item_list.append(payment_method)
				data_categories.append(item_list)
		del data
	print("Categories:")
	...
	parquet_output = os.path.join(data_file, f"data_high_value.parquet");
	converted_data.to_parquet(parquet_output)


def data_process_high_value():
	category_mp, price_mp = product_catalog_read("data/product_catalog.json")
	data_process_high_value_sub("data/30G_data_new", 16, category_mp, price_mp)


def main():
	start_time = time.time()
	data_process_high_value()
	end_time = time.time()
